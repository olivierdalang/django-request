...
class RequestTests(TestCase):
    ...
    def test_browser_detection_with_no_path(self):
        ...
        request = Request(
            user_agent=(
                "Mozilla/5.0 (compatible; MSIE 9.0; America Online Browser 1.1; Windows"
                " NT 5.0)"
            )
        )
    ...
    def test_determining_search_keywords(self):
        request = Request(
            referer=(
                ...
                "client=safari&rls=en&q=querykit+core+data&ie=UTF-8&oe=UTF-8"
            )
        )
        self.assertEqual(request.keywords, "querykit core data")

    @mock.patch(
        "request.models.gethostbyaddr", return_value=("foo.net", [], ["1.2.3.4"])
    )
    ...
    @mock.patch("request.models.request_settings.LOG_IP", False)
    def test_save_not_log_ip(self):
        request = Request(ip="1.2.3.4")
        request.save()
        self.assertEqual(settings.IP_DUMMY, request.ip)

    @mock.patch("request.models.request_settings.ANONYMOUS_IP", True)
    def test_save_anonymous_ip(self):
        request = Request(ip="1.2.3.4")
        request.save()
        self.assertTrue(request.ip.endswith(".1"))

    @mock.patch("request.models.request_settings.LOG_USER", False)
